...
```bash
define service {
# NOME DO HOST AO QUAL O SERVIÇO PERTENCE.
    host_name
    hostgroup_name					# hostgroup_name.
# DESCRIÇÃO DO SERVIÇO (Nome do serviço).
    service_description
# DESCRIÇÂO PARA O DISPLAY DO SERVIÇO.
    display_name
    parents							# service_descriptions
    import
    servicegroups					# Grupo de serviços.
    is_volatile						[0/1]
# Comando que será executado para esse serviço.
    check_command
...
# Nome do periodo que as verificações ativas serão  feitas.
    ...
    notifications_enabled			[0/1]
    contacts						# Contatos para notificações.
# Grupo de contatos para notificações.
    contact_groups
